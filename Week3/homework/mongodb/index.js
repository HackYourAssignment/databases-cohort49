...
async function createEpisodeExercise(client) {
  ...
  const result = await client.db("databaseWeek3").collection("bob_ross_episodes").insertOne(episodeData);
...
async function updateEpisodeExercises(client) {
  ...
  const updateEpisode13Season30 = await collection.updateOne(
    { episode: "S30E13" },
    { $set: { title: "BLUE RIDGE FALLS" } }
  );
  console.log(`Ran a command to update episode 13 in season 30 and it updated ${updateEpisode13Season30.modifiedCount} episodes`);

  const updateBushesToBush = await collection.updateMany(
    { elements: "BUSHES" },
    { $set: { "elements.$": "BUSH" } }
  );
  console.log(`Ran a command to update all the BUSHES to BUSH and it updated ${updateBushesToBush.modifiedCount} episodes`);
}
